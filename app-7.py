...
:root {
  --primary:#152B50; --accent1:#52B8DE; --accent2:#E25D32;
  --accent3:#F3B73F; --accent4:#8AB738;
  --surface:#FFFFFF; --border:#DDE3F0; --muted:#6B7A99;
  --radius:12px; --font-h:'Montserrat',sans-serif; --font-b:'Montserrat',sans-serif;
}
html, body, [class*="css"] { font-family:var(--font-b) !important; }
.block-container { max-width:820px !important; padding:0 1.2rem 3rem !important; }
section[data-testid="stSidebar"] { display:none; }
#MainMenu, footer, header { visibility:hidden; }

/* ── Header banner ── */
.rp-header {
  background:#152B50; border-radius:0 0 16px 16px;
  padding:1.8rem 2rem 1.6rem; margin-bottom:1.4rem;
  position:relative; overflow:hidden;
}
.rp-header::before {
  content:''; position:absolute; inset:0;
  background:repeating-linear-gradient(135deg,transparent,transparent 28px,
    rgba(255,255,255,.03) 28px,rgba(255,255,255,.03) 30px);
}
.rp-header-inner { display:flex; align-items:center; justify-content:space-between; position:relative; }
.rp-header-bar { position:absolute; bottom:0; left:0; right:0; height:3px; }
.kids-bar { background:linear-gradient(to right,#E25D32,#F3B73F,#52B8DE,#8AB738); }
.yth-bar  { background:#1EBFE1; }

/* ── Camp type tabs (display only) ── */
.camp-tabs { display:flex; gap:8px; margin-bottom:1.2rem; }
.camp-tab-pill {
  flex:1; padding:.65rem 1rem; border-radius:10px;
  border:2px solid #DDE3F0; background:#fff; text-align:center;
  font-family:var(--font-h); font-weight:700; font-size:.84rem; color:#6B7A99;
  position:relative; overflow:hidden;
}
.camp-tab-pill::after { content:''; position:absolute; top:0; left:0; right:0; height:3px; }
.kids-tab { border-color:#152B50 !important; color:#152B50 !important; }
.kids-tab::after { background:linear-gradient(to right,#E25D32,#F3B73F,#52B8DE,#8AB738); }
.yth-tab  { border-color:#1E1E1E !important; background:#1E1E1E !important; color:#fff !important; }
.yth-tab::after  { background:#1EBFE1; }

/* ── Section headers ── */
.sec-head { display:flex; align-items:center; gap:.65rem; margin:1.4rem 0 .65rem; }
.sec-num {
  width:24px; height:24px; border-radius:50%; background:#52B8DE; color:#152B50;
...
.sec-title {
  ...
  text-transform:uppercase; letter-spacing:1.6px; color:#6B7A99;
}

/* ── Upload cards ── */
.upload-card {
  background:#fff; border:1.5px solid #DDE3F0; border-radius:var(--radius);
  padding:1rem 1.2rem .8rem; position:relative; margin-bottom:.4rem;
}
.upload-card::before {
  content:''; position:absolute; top:0; left:0;
  width:4px; height:100%; border-radius:var(--radius) 0 0 var(--radius);
}
.uc-reg::before { background:#52B8DE; }
.uc-cg::before  { background:#E25D32; }
.uc-ldr::before { background:#8AB738; }
.uc-supp::before{ background:#F3B73F; }
.uc-opt { border-style:dashed; }
.upload-label {
  font-family:var(--font-h); font-size:.65rem; font-weight:700;
  text-transform:uppercase; letter-spacing:1.2px; color:#6B7A99;
  margin-bottom:.3rem; display:flex; align-items:center; gap:.4rem;
}
.badge-req { background:#52B8DE; color:#152B50; border-radius:4px; padding:1px 6px; font-size:.58rem; font-weight:900; }
.badge-rec { background:#E8F5E9; color:#2e7d32; border-radius:4px; padding:1px 6px; font-size:.58rem; font-weight:900; }
.badge-opt { background:#DDE3F0; color:#6B7A99;  border-radius:4px; padding:1px 6px; font-size:.58rem; font-weight:900; }
.upload-desc { font-size:.74rem; color:#6B7A99; margin-bottom:.6rem; line-height:1.4; }
...
.kids-param { border-left:4px solid #52B8DE; }
.yth-param  { border-top:3px solid #1EBFE1; }
.param-card h4 {
  font-family:var(--font-h); font-size:.65rem; font-weight:700;
  text-transform:uppercase; letter-spacing:1.3px; color:#6B7A99;
  margin:0 0 .8rem; padding-bottom:.5rem; border-bottom:1px solid #DDE3F0;
}

/* ── Banners ── */
.ok-banner   { background:#F0FDF4; border-left:4px solid #8AB738; border-radius:0 8px 8px 0; padding:.65rem 1rem; color:#14532d; font-size:.83rem; margin:.4rem 0; }
.warn-banner { background:#FFFBEB; border-left:4px solid #F3B73F; border-radius:0 8px 8px 0; padding:.65rem 1rem; color:#7d5a00; font-size:.83rem; margin:.4rem 0; }
.info-banner { background:#EFF8FF; border-left:4px solid #52B8DE; border-radius:0 8px 8px 0; padding:.65rem 1rem; color:#0c5c82; font-size:.83rem; margin:.4rem 0; }

/* ── Stats ── */
.stats-row { display:grid; grid-template-columns:repeat(4,1fr); gap:10px; margin:1.2rem 0; }
.stat-box { background:#fff; border:1.5px solid #DDE3F0; border-radius:var(--radius); padding:14px 8px; text-align:center; position:relative; overflow:hidden; }
.stat-box::after { content:''; position:absolute; bottom:0; left:0; right:0; height:3px; background:#52B8DE; opacity:.35; }
.stat-box .val { font-family:var(--font-h); font-size:1.8rem; font-weight:900; color:#152B50; line-height:1; }
.stat-box .lbl { font-size:.62rem; color:#6B7A99; margin-top:5px; text-transform:uppercase; letter-spacing:.8px; font-weight:700; }
.stat-box.warn { border-color:#F3B73F; } .stat-box.warn .val { color:#b8600a; } .stat-box.warn::after { background:#F3B73F; opacity:1; }
.stat-box.good { border-color:#8AB738; } .stat-box.good .val { color:#2e7d32; } .stat-box.good::after { background:#8AB738; opacity:1; }

/* ── Results table ── */
.grade-table { width:100%; border-collapse:collapse; font-size:.84rem; margin-top:.5rem; }
.grade-table th { background:#152B50; color:#fff; padding:9px 14px; text-align:left; font-family:var(--font-h); font-weight:700; font-size:.67rem; letter-spacing:1px; text-transform:uppercase; }
.grade-table td { padding:9px 14px; border-bottom:1px solid #DDE3F0; }
.grade-table tr:last-child td { border-bottom:none; }
.grade-table tr:hover td { background:#f8faff; }
.grade-dot { display:inline-block; width:8px; height:8px; border-radius:50%; margin-right:7px; }

/* ── Camp selector buttons ── */
/* KIDS button (primary when selected) */
div[data-testid="stButton"]:has(button[data-testid="btn_kids"]) button[kind="primary"] {
  background: #152B50 !important; color: #fff !important;
  border: 2px solid #152B50 !important; border-radius: 10px !important;
  font-family: 'Montserrat',sans-serif !important; font-weight: 700 !important;
  font-size: .9rem !important; position: relative !important; overflow: hidden !important;
}
div[data-testid="stButton"]:has(button[data-testid="btn_kids"]) button[kind="secondary"] {
  background: #fff !important; color: #6B7A99 !important;
  border: 2px solid #DDE3F0 !important; border-radius: 10px !important;
  font-family: 'Montserrat',sans-serif !important; font-weight: 700 !important;
  font-size: .9rem !important;
}
/* YTH button (primary when selected) */
div[data-testid="stButton"]:has(button[data-testid="btn_yth"]) button[kind="primary"] {
  background: #1E1E1E !important; color: #fff !important;
  border: 2px solid #1EBFE1 !important; border-radius: 10px !important;
  font-family: 'Inter',sans-serif !important; font-weight: 700 !important;
  font-size: .9rem !important;
}
div[data-testid="stButton"]:has(button[data-testid="btn_yth"]) button[kind="secondary"] {
  background: #fff !important; color: #6B7A99 !important;
  border: 2px solid #DDE3F0 !important; border-radius: 10px !important;
  font-family: 'Inter',sans-serif !important; font-weight: 700 !important;
  font-size: .9rem !important;
...
.stDownloadButton > button { width:100% !important; background:#152B50 !important; color:#fff !important; border:none !important; border-radius:8px !important; padding:.8rem 1rem !important; font-family:var(--font-h) !important; font-size:.9rem !important; font-weight:700 !important; letter-spacing:.5px !important; margin-top:.5rem; }
.stDownloadButton > button:hover { background:#1E3E72 !important; }
.stButton > button[kind="primary"] { background:#152B50 !important; color:#fff !important; border:none !important; border-radius:8px !important; font-family:var(--font-h) !important; font-weight:700 !important; letter-spacing:.5px !important; }
.stButton > button[kind="primary"]:hover { background:#1E3E72 !important; }

/* ── Misc ── */
[data-testid="stFileUploader"] { border:1.5px dashed #DDE3F0 !important; border-radius:10px !important; }
hr { border-color:#DDE3F0 !important; margin:1rem 0 !important; }
div[data-testid="stRadio"] label { font-size:.9rem !important; }
div[data-testid="stToggle"] label { font-size:.9rem !important; }
[data-testid="stSlider"] [role="slider"] { background:#52B8DE !important; }
</style>
...
# SVG LOGOS
# ══════════════════════════════════════════════════════════════════════════════
KIDS_LOGO_SVG = '''<svg viewBox="0 0 230 72" xmlns="http://www.w3.org/2000/svg" style="height:56px;width:auto;display:block;">
  <text x="2" y="13" font-family="Montserrat,sans-serif" font-weight="900" font-size="9" letter-spacing="3.5" fill="rgba(255,255,255,0.5)">ROCK POINT</text>
  <text x="2" y="65" font-family="Montserrat,sans-serif" font-weight="900" font-size="52" fill="#E25D32">K</text>
  <line x1="18" y1="18" x2="36" y2="65" stroke="#F3B73F" stroke-width="4.5" stroke-linecap="round"/>
  <text x="42" y="65" font-family="Montserrat,sans-serif" font-weight="900" font-size="52" fill="#F3B73F">I</text>
  <text x="60" y="65" font-family="Montserrat,sans-serif" font-weight="900" font-size="52" fill="#52B8DE">D</text>
  <text x="103" y="65" font-family="Montserrat,sans-serif" font-weight="900" font-size="52" fill="#8AB738">S</text>
</svg>'''

YTH_LOGO_SVG = '''<svg viewBox="0 0 190 72" xmlns="http://www.w3.org/2000/svg" style="height:56px;width:auto;display:block;">
  <circle cx="15" cy="16" r="13" fill="#1EBFE1"/>
  <line x1="15" y1="7"  x2="15" y2="25" stroke="#1E1E1E" stroke-width="3.2" stroke-linecap="round"/>
  <line x1="9"  y1="13" x2="21" y2="13" stroke="#1E1E1E" stroke-width="3.2" stroke-linecap="round"/>
  <text x="35" y="28" font-family="Inter,Helvetica Neue,sans-serif" font-weight="900" font-size="26" letter-spacing="-1" fill="#FFFFFF">YTH</text>
  <text x="35" y="44" font-family="Inter,Helvetica Neue,sans-serif" font-weight="400" font-size="8.5" letter-spacing="3" fill="rgba(255,255,255,0.45)">ROCK POINT</text>
</svg>'''
...
def file_ok_banner(label, df):
    st.markdown(f'<div style="padding:0 1rem;"><div class="ok-banner">✅ <strong>{label}</strong> — '
                ...
                unsafe_allow_html=True)
# ══════════════════════════════════════════════════════════════════════════════
# STEP 1 — CAMP TYPE  (columns + buttons — reliable in Streamlit)
# ══════════════════════════════════════════════════════════════════════════════

# Initialize session state default
if 'camp_type' not in st.session_state:
    st.session_state['camp_type'] = 'KIDS'

# Render two big styled columns as the selector
col_k, col_y = st.columns(2)
with col_k:
    if st.button("🏕️  KIDS Camp", use_container_width=True, key='btn_kids',
                  type='primary' if st.session_state['camp_type'] == 'KIDS' else 'secondary'):
        st.session_state['camp_type'] = 'KIDS'
        st.rerun()
with col_y:
    if st.button("✝️  YTH Camp", use_container_width=True, key='btn_yth',
                  type='primary' if st.session_state['camp_type'] == 'YTH' else 'secondary'):
        st.session_state['camp_type'] = 'YTH'
        st.rerun()

camp_type = st.session_state['camp_type']

_is_yth = (camp_type == 'YTH')
_seccls = 'yth-sec' if _is_yth else ''
_pcls   = 'yth-param' if _is_yth else 'kids-param'
_font   = 'Inter,sans-serif' if _is_yth else 'Montserrat,sans-serif'
_logo   = YTH_LOGO_SVG if _is_yth else KIDS_LOGO_SVG
_tagline = (
    "Creating purposeful environments that are community driven"
    if _is_yth else
    "Helping kids learn about a God they can't see through relationships with people they can see"
)
...
<div class="rp-header">
  <div class="rp-header-inner">
    ...
    <div style="text-align:right;">
      <div style="font-family:{_font};font-size:.58rem;font-weight:700;letter-spacing:2.5px;
           text-transform:uppercase;color:rgba(255,255,255,.32);margin-bottom:.3rem;">GROUP SORTER</div>
      <div style="font-family:{_font};font-size:.7rem;color:rgba(255,255,255,.38);
           font-style:italic;max-width:230px;line-height:1.45;text-align:right;">{_tagline}</div>
    </div>
  </div>
  <div class="rp-header-bar {'yth-bar' if _is_yth else 'kids-bar'}"></div>
</div>
""", unsafe_allow_html=True)

# ── UI helper functions ────────────────────────────────────────────────────────
def sec_header(num, title, note=''):
    note_html = (f'<span style="font-weight:400;text-transform:none;letter-spacing:0;'
                 f'font-size:.74rem;color:#aaa;margin-left:.3rem;">{note}</span>') if note else ''
    st.markdown(f"""
<div class="sec-head {_seccls}">
  <div class="sec-num">{num}</div>
  <div class="sec-title">{title}{note_html}</div>
</div>""", unsafe_allow_html=True)

def param_open(title):
    st.markdown(f'<div class="param-card {_pcls}"><h4>{title}</h4>', unsafe_allow_html=True)

def param_close():
    st.markdown('</div>', unsafe_allow_html=True)

def upload_card_open(slot_cls, label, badge_html, desc, opt=False):
    opt_cls = 'uc-opt' if opt else ''
    st.markdown(f"""
<div class="upload-card {slot_cls} {opt_cls}">
  <div class="upload-label">{label}&nbsp;{badge_html}</div>
  <div class="upload-desc">{desc}</div>""", unsafe_allow_html=True)
...
if has_grade and detected_grades:
    param_open("Grade Filter <span style='font-weight:400;text-transform:none;letter-spacing:0;font-size:.75rem;color:#aaa;'>(optional — leave all selected to include everyone)</span>")
...
param_open("Group Sizing Priority")
